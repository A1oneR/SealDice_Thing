...
// OnGroupUpload 群文件上传事件处理 (OneBot 11 notice_type: group_upload)，其他 Adapter 可迁移至此
func (s *IMSession) OnGroupUpload(ctx *MsgContext, msg *Message, file *GroupUploadFile) {
	groupInfo, ok := s.ServiceAtNew.Load(msg.GroupID)
	if !ok {
		s.Parent.Logger.Debugf("group_upload: 群 %s 不在 ServiceAtNew 中，跳过扩展回调", msg.GroupID)
		return
	}
	for _, i := range groupInfo.ActivatedExtList {
		if i.OnGroupUpload != nil {
			i.callWithJsCheck(ctx.Dice, func() {
				i.OnGroupUpload(ctx, msg, file)
			})
		}
	}
}
