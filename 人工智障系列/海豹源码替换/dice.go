...
// GroupUploadFile OneBot 11 群文件上传事件中的文件信息 (notice_type: group_upload)
type GroupUploadFile struct {
	Id    string `json:"id" jsbind:"id"`       // 文件 ID
	Name  string `json:"name" jsbind:"name"`   // 文件名
	Size  int64  `json:"size" jsbind:"size"`   // 文件大小（字节）
	Busid int64  `json:"busid" jsbind:"busid"` // busid
}
...
type ExtInfo struct {
	...
	OnGroupUpload       func(ctx *MsgContext, msg *Message, file *GroupUploadFile) `yaml:"-" json:"-" jsbind:"onGroupUpload"` // OneBot 11 群文件上传通知
