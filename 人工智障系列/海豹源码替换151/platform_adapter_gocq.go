...
type MessageQQBase struct {
	...
	// group_upload 事件中的文件信息 (notice_type: group_upload)
	File *struct {
		Id    string `json:"id"`
...
func (pa *PlatformAdapterGocq) Serve() int {
	...
	socket.OnTextMessage = func(message string, socket gowebsocket.Socket) {
		...
		// 群文件上传 — 必须在 Unmarshal 之前用 gjson 判断并处理，否则 LLOneBot 等格式可能导致 Unmarshal 失败而提前 return，永远进不到此分支
		isGroupUpload := (gjson.Get(message, "post_type").String() == "notice" && gjson.Get(message, "notice_type").String() == "group_upload") ||
			gjson.Get(message, "detail_type").String() == "notice.group_upload" ||
			(gjson.Get(message, "type").String() == "notice" && gjson.Get(message, "detail_type").String() == "group_upload")
		if isGroupUpload {
			root := gjson.Parse(message)
			if !root.Get("group_id").Exists() && root.Get("payload").Exists() {
				root = root.Get("payload")
			}
			groupIDRaw := root.Get("group_id")
			if !groupIDRaw.Exists() || groupIDRaw.String() == "" {
				groupIDRaw = root.Get("data.group_id")
			}
			if !groupIDRaw.Exists() || groupIDRaw.String() == "" {
				groupIDRaw = root.Get("detail.group_id")
			}
			userIDRaw := root.Get("user_id")
			if !userIDRaw.Exists() || userIDRaw.String() == "" {
				userIDRaw = root.Get("data.user_id")
			}
			if !userIDRaw.Exists() || userIDRaw.String() == "" {
				userIDRaw = root.Get("detail.user_id")
			}
			fileObj := root.Get("file")
			if !fileObj.Exists() || !fileObj.IsObject() {
				fileObj = root.Get("data.file")
			}
			if !fileObj.Exists() || !fileObj.IsObject() {
				fileObj = root.Get("detail.file")
			}
			groupID := FormatDiceIDQQGroup(groupIDRaw.String())
			userID := FormatDiceIDQQ(userIDRaw.String())
			log.Infof("group_upload 事件: group_id=%s user_id=%s file=%s", groupID, userID, fileObj.Get("name").String())
			msg := new(Message)
			msg.MessageType = "group"
			msg.GroupID = groupID
			msg.Sender.UserID = userID
			timeVal := root.Get("time")
			if timeVal.Exists() {
				msg.Time = timeVal.Int()
			}
			msg.Platform = "QQ"
			...
			var fileInfo *GroupUploadFile
			if fileObj.Exists() && fileObj.IsObject() {
				name := fileObj.Get("name").String()
				if name == "" {
					name = fileObj.Get("filename").String()
				}
				if name == "" {
					name = fileObj.Get("file_name").String()
				}
				fileInfo = &GroupUploadFile{
					Id:    fileObj.Get("id").String(),
					Name:  name,
					Size:  fileObj.Get("size").Int(),
					Busid: fileObj.Get("busid").Int(),
				}
			}
			session.OnGroupUpload(ctx, msg, fileInfo)
			return
		}
