...
async function processLogTask(ctx, msg, cmdArgs, modeName, pythonMode) {
    ...
    if (!linkArgs.length) {
        const bare = args.filter(a => !excludeList.includes(a) && !excludeList.includes(a.toLowerCase()) && !customPrompts[a]);
        if (bare.length === 1) linkArgs = bare;
    }
    let logSources = linkArgs.map(parseLogLinkToken).filter(Boolean);
    let useLocalFile = !logSources.length;
    ...
    if (useLocalFile) {
        ...
        if (!downloadUrl) {
            seal.replyToSender(ctx, msg, `❌ 获取群文件下载链接失败，请确认 OneBot API 地址或文件消息中包含可用直链。`);
            return seal.ext.newCmdExecuteResult(true);
        }
